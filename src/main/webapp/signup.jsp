...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .toast{
    ...
    transition:transform .25s ease, opacity .25s ease;pointer-events:none;max-width:320px;
  }
...
<script>
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (valid){
      showToast(`Welcome to Denim Co., ${document.getElementById('firstName').value || 'friend'}!`);
...
console.log("Sending req: ",raw);
document.getElementById('createAccountBtn').addEventListener("click", function(event) {
  event.preventDefault();
  console.log(requestOptions);
 fetch("http://localhost:8081/Bloomster/create-account", requestOptions)
    .then((response) => {
      console.log("HTTP Status received:", response.status);
      if (!response.ok) {
        throw new Error(`HTTP error! Status: ${response.status}`);
      }
      window.location.href = "otp.jsp?email=" + encodeURIComponent(email);
      return response.json(); 
    })
    .then((data) => {
      // 3. This is your parsed backend JSON data
...
if(res.status === 200){
  window.location.href = "Verifyotp.jsp";
      ...
      //<%-- form.reset(); --%>
      bars.forEach(bar => bar.style.background = '#e7e7ea');
      pwHint.textContent = 'Use 8+ characters with a number and a symbol.';
    }
  });
